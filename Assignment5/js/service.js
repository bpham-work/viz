...
class AssignmentService {
    ...
    generateDataGrid(NX, NY, NZ, colorCmd=undefined, colorArgs=undefined) {
        ...
        for (let xIndex = 0; xIndex < NX; xIndex++) {
            ...
            for (let yIndex = 0; yIndex < NY; yIndex++) {
                ...
                for (let zIndex = 0; zIndex < NZ; zIndex++) {
                    ...
                    let s = this.tempGenerator.getTemp(x, y, z);
                    let newNode = new Node(x, y, z, s);
                    if (colorCmd) {
                        colorArgs.s = s;
                        let rgb = colorCmd(colorArgs);
                        newNode.setRGB(rgb);
                    }
